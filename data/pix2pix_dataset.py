...
#            image = image.filter(ImageFilter.GaussianBlur(radius=3.5))
            ...
            guus_image = np.array(guus_image)
            imageblur = cv2.GaussianBlur(guus_image, (3, 3), 3)
            imageblur = Image.fromarray(imageblur)
            #imageblur = imageblur.convert('RGB')
            # print(self.opt.preprocess_mode)

            transform_image = get_transform(self.opt, params)
            image_tensor = transform_image(image)
            image_blur_tensor = transform_image(imageblur)
