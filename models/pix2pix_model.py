...
class Pix2PixModel(nn.Module):
    ...
    def execute(self, data, mode):
     #  import pdb;pdb.set_trace()
        input_semantics, label_existence, real_image, real_edge,real_image_blur = self.preprocess_input(data)
        # print("input_semantics: " , input_semantics.shape)
        # print("real_image: ", real_image.shape)
        # exit(0)

        if mode == 'generator':
            g_loss, generated, generated2, generated_edge = self.compute_generator_loss(input_semantics, label_existence, real_image, real_edge,real_image_blur)
            return g_loss, generated, generated2, generated_edge
        elif mode == 'discriminator':
            d_loss = self.compute_discriminator_loss(input_semantics, label_existence, real_image, real_edge,real_image_blur)
            return d_loss
    ...
    def preprocess_input(self, data):
        ...
        # print(target_label_existence.size()) [4, 256, 512]
        tvect = jt.zeros(bs, self.opt.label_nc)
        for i in range(bs):
            hist = jt.histc(jt.float32(target_label_existence[i]),
                               bins=self.opt.label_nc, min=0, max=self.opt.label_nc - 1)
            vect = hist > 0
            tvect[i] = vect
        ...
        return input_semantics,tvect, data['image'],data['edge'],data['image_blur']

    def compute_generator_loss(self, input_semantics, label_existence, real_image,real_edge, real_image_blur):
        G_losses = {}
        ...
        G_losses['GAN'] = (self.criterionGAN(pred_fake, True, for_discriminator=False) + \
                          self.criterionGAN(pred_fake_edge, True, for_discriminator=False) + \
                          self.criterionGAN(pred_fake2, True, for_discriminator=False)*3)
        ...
        if not self.opt.no_ganFeat_loss:
            ...
            for i in range(num_D):  # for each discriminator
                ...
                for j in range(num_intermediate_outputs):  # for each layer output
                    ...
                    GAN_Feat_loss += (unweighted_loss + unweighted_loss2 + unweighted_loss_edge) * self.opt.lambda_feat / num_D
            G_losses['GAN_Feat'] = GAN_Feat_loss

        if not self.opt.no_vgg_loss:
            G_losses['VGG'] = (self.criterionVGG(fake_image, real_image) * self.opt.lambda_vgg + \
                              self.criterionVGG(fake_edge, real_edge) * self.opt.lambda_vgg + \
                              self.criterionVGG(fake_image2, real_image) * self.opt.lambda_vgg)*3

        return G_losses, fake_image, fake_image2, fake_edge

    def compute_discriminator_loss(self, input_semantics, label_existence,real_image,real_edge,real_image_blur):
        D_losses = {}
        with jt.no_grad():
            fake_image, fake_image2, fake_edge, _ = self.generate_fake(input_semantics, label_existence, real_image)
        ...
        D_losses['D_Fake'] = self.criterionGAN(pred_fake, False, for_discriminator=True) + \
                             self.criterionGAN(pred_fake_edge, False, for_discriminator=True) + \
                             self.criterionGAN(pred_fake2, False, for_discriminator=True)*3
        D_losses['D_real'] = self.criterionGAN(pred_real, True, for_discriminator=True) + \
                             self.criterionGAN(pred_real_edge, True, for_discriminator=True) + \
                             self.criterionGAN(pred_real2, True, for_discriminator=True)*3
