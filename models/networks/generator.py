...
class SPADEGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        # self.model_edge=nn.Sequential(
        #     nn.Conv2d(final_nc, 32, 3, padding=1),
        #     nn.Sigmoid(),
    ...
    def execute(self, input,label_existence, z=None):
        ...
        edge1 = self.conv_edge1(nn.leaky_relu(x, 2e-1))
        #import pdb;pdb.set_trace()
        edge1_att = jt.sigmoid(edge1)
        edge2 = self.conv_edge2(nn.leaky_relu(edge1, 2e-1))
        edge2_att = jt.sigmoid(edge2)
        edge3 = self.conv_edge3(nn.leaky_relu(edge2, 2e-1))
        edge3_att = jt.sigmoid(edge3)
        edge4 = self.conv_edge4(nn.leaky_relu(edge3, 2e-1))
        edge_att = jt.sigmoid(jt.tanh(edge4))
        edge = jt.tanh(edge4)

        image1 = self.conv_img1(nn.leaky_relu(x, 2e-1))
        image1 = image1 * edge1_att + image1
        image2 = self.conv_img2(nn.leaky_relu(image1, 2e-1))
        image2 = image2 * edge2_att + image2
        image3 = self.conv_img3(nn.leaky_relu(image2, 2e-1))
        image3 = image3 * edge3_att + image3
        image4 = self.conv_img4(nn.leaky_relu(image3, 2e-1))
        image = jt.tanh(image4)
        image = image * edge_att + image

        feature_image_edge_seg_combine = jt.cat((x, image, edge, seg), 1)
        class_feature = self.conv_comb1(nn.leaky_relu(feature_image_edge_seg_combine, 2e-1))
        ...
        refined_class_feature = self.conv_comb2(nn.leaky_relu(refined_class_feature, 2e-1))
        feature_image_edge_seg_combine = self.conv_comb(nn.leaky_relu(refined_class_feature, 2e-1))
        image2 = jt.tanh(feature_image_edge_seg_combine)     
        # print('image2', image2.size()) [4, 3, 256, 512]
        return image, image2, edge
#        return filtered_image, filtered_image2, edge
